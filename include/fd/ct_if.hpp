...
namespace boost {
  ...
  struct true_type_t  { enum { value = true }; };
  struct false_type_t { enum { value = false }; };

  template <class A, class B>
  struct ct_and { typedef false_type_t type; };
  template <> struct ct_and<true_type_t,true_type_t> { typedef true_type_t type; };

  template <class A> struct ct_not { typedef ct_if_error type; };
  template <> struct ct_not<true_type_t> { typedef false_type_t type; };
  template <> struct ct_not<false_type_t> { typedef true_type_t type; };
...
// 'ct_if' non-type template parameter boolean.
  ...
  template <class A, class B>
  struct ct_if_t<true_type_t, A, B> { typedef A type; };
  template <class A, class B>
  struct ct_if_t<false_type_t, A, B> { typedef B type; };
